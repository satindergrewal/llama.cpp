    ...
    const std::vector<metric_item> counters = {
        {
            ...
            "reval_saves_total",
            "Prompt-cache entries sealed with a state<->claim binding (DS4P_REVALIDATE)",
            (double) metrics.n_reval_saves
        }, {
            "reval_loads_total",
            "Prompt-cache loads that passed binding verification (DS4P_REVALIDATE)",
            (double) metrics.n_reval_loads
        }, {
            "reval_hash_fail_total",
            "Prompt-cache loads rejected on payload hash mismatch (degraded to recompute)",
            (double) metrics.n_reval_hash_fail
        }, {
            "reval_identity_fail_total",
            "Prompt-cache loads rejected on identity mismatch (degraded to recompute)",
            (double) metrics.n_reval_identity_fail
        }, {
            "reval_cell_mismatch_total",
            "Prompt-cache saves refused: live KV position range != token claim",
            (double) metrics.n_reval_cell_mismatch
        }, {
            "quench_seqs_total",
            "Requests terminally quenched by the yield-quench governor (DS4P_YIELD_QUENCH)",
            (double) metrics.n_quench_seqs_total
        },
    };
