...
static const struct ggml_type_traits_cpu type_traits_cpu[GGML_TYPE_COUNT] = {
    ...
    // IQK quant types (ported from ik_llama.cpp): reference vec_dot for now,
    // optimized kernels are added separately
    [GGML_TYPE_IQ4_K] = {
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_FLASH_ATTN_EXT_DSA:
            {
                // gathered top-k attention is implemented in the CUDA backend only
                GGML_ABORT("GGML_OP_FLASH_ATTN_EXT_DSA has no CPU implementation");
            } break;
