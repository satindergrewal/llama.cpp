...
static constexpr __host__ __device__ int get_vdr_mmvq(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_IQ4_KT:  return VDR_IQ4_KT_Q8_1_MMVQ;
        case GGML_TYPE_IQ1_KT:  return VDR_IQ1_KT_Q8_1_MMVQ;
        case GGML_TYPE_IQ5_KS:  return VDR_IQ5_KS_Q8_1_MMVQ;
        case GGML_TYPE_IQ3_KT:  return VDR_IQ3_KT_Q8_1_MMVQ;
        case GGML_TYPE_IQ2_KT:  return VDR_IQ2_KT_Q8_1_MMVQ;
...
#pragma unroll
    ...
    const int64_t kbx_offset = int64_t(sample_x)*stride_sample_x + int64_t(channel_x)*stride_channel_x + int64_t(row0)*stride_row_x;
...
#pragma unroll
            for (int i = 0; i < rows_per_cuda_block; ++i) {
                if constexpr (ggml_cuda_iqk_row_meta_size(type) > 0) {
                    // Row-meta layout: strides arrive in BYTES; hand vec_dot the ROW BASE and a
                    // WITHIN-ROW block index. vec_dot reads the leading f32 row scale itself.
                    const char * row_base_x = (const char *) vx + (size_t) (kbx_offset + i*stride_row_x);
                    tmp[j][i] += vec_dot_q_cuda(row_base_x, &y[j*stride_col_y + kby], kbx, kqs);
                    if constexpr (has_fusion) {
                        if (use_gate) {
                            const char * row_base_g = (const char *) vgate + (size_t) (kbx_offset + i*stride_row_x);
                            tmp_gate[j][i] += vec_dot_q_cuda(row_base_g, &y[j*stride_col_y + kby], kbx, kqs);
                        }
                    }
                } else {
...
#pragma unroll
        for (int i = 0; i < c_rows_per_block; ++i) {
            if constexpr (ggml_cuda_iqk_row_meta_size(type) > 0) {
                // Row-meta layout: strides in BYTES, vec_dot gets ROW BASE + within-row block index.
                // This is the path GLM-class MoE experts take (IQ1_KT), so it matters most here.
                const char * row_base_x = (const char *) vx + (size_t) (kbx_offset + i*stride_row_x);
                tmp[i] += vec_dot_q_cuda(row_base_x, &y[kby], kbx, kqs);
            } else {
...
static void mul_mat_vec_q_switch_type(
    ...
    switch (type_x) {
        ...
        case GGML_TYPE_IQ4_K:
            mul_mat_vec_q_switch_ncols_dst<GGML_TYPE_IQ4_K>
                (vx, vy, ids, fusion, dst, ncols_x, nrows_x, ncols_dst, stride_row_x, stride_col_y, stride_col_dst,
                 nchannels_x, nchannels_y, nchannels_dst, stride_channel_x, stride_channel_y, stride_channel_dst,
                 nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, stream);
            break;
        case GGML_TYPE_IQ4_KT:
            mul_mat_vec_q_switch_ncols_dst<GGML_TYPE_IQ4_KT>
                (vx, vy, ids, fusion, dst, ncols_x, nrows_x, ncols_dst, stride_row_x, stride_col_y, stride_col_dst,
                 nchannels_x, nchannels_y, nchannels_dst, stride_channel_x, stride_channel_y, stride_channel_dst,
                 nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, stream);
            break;
        case GGML_TYPE_IQ1_KT:
            mul_mat_vec_q_switch_ncols_dst<GGML_TYPE_IQ1_KT>
                (vx, vy, ids, fusion, dst, ncols_x, nrows_x, ncols_dst, stride_row_x, stride_col_y, stride_col_dst,
                 nchannels_x, nchannels_y, nchannels_dst, stride_channel_x, stride_channel_y, stride_channel_dst,
                 nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, stream);
            break;
        case GGML_TYPE_IQ5_KS:
            mul_mat_vec_q_switch_ncols_dst<GGML_TYPE_IQ5_KS>
                (vx, vy, ids, fusion, dst, ncols_x, nrows_x, ncols_dst, stride_row_x, stride_col_y, stride_col_dst,
                 nchannels_x, nchannels_y, nchannels_dst, stride_channel_x, stride_channel_y, stride_channel_dst,
                 nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, stream);
            break;
        ...
        case GGML_TYPE_IQ6_K:
            mul_mat_vec_q_switch_ncols_dst<GGML_TYPE_IQ6_K>
                (vx, vy, ids, fusion, dst, ncols_x, nrows_x, ncols_dst, stride_row_x, stride_col_y, stride_col_dst,
                 nchannels_x, nchannels_y, nchannels_dst, stride_channel_x, stride_channel_y, stride_channel_dst,
                 nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, stream);
            break;
...
void ggml_cuda_mul_mat_vec_q(
    ...
    // Row-meta types (the IQK trellis family) carry a per-row f32 header, so nb[1] is NOT an
    // exact multiple of the type size and nb[1]/ts_src0 would silently TRUNCATE the header away
    // (e.g. IQ4_KT: nb[1] = 4 + nblocks*128, ts = 128  =>  the +4 is lost).
    // For those types we pass the raw BYTE strides in the same slots; the mmvq kernels detect
    // this at compile time via ggml_cuda_iqk_row_meta_size(type) and do byte arithmetic,
    // handing vec_dot a ROW BASE plus a within-row block index (ik's calling convention).
    const bool    x_row_meta = ggml_row_meta_size(src0->type) != 0;
    const int64_t s01 = x_row_meta ? (int64_t) src0->nb[1] : (int64_t) (src0->nb[1] / ts_src0);
    const int64_t s11 = ne10_padded / QK8_1;
    const int64_t s1  =  dst->nb[1] / ts_dst;
    const int64_t s02 = x_row_meta ? (int64_t) src0->nb[2] : (int64_t) (src0->nb[2] / ts_src0);
    const int64_t s2  =  dst->nb[2] / ts_dst;
    const int64_t s03 = x_row_meta ? (int64_t) src0->nb[3] : (int64_t) (src0->nb[3] / ts_src0);
