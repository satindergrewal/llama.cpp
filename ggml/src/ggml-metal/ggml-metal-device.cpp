...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_sinkhorn_norm(ggml_metal_library_t lib, const ggml_tensor * op) {
    assert(op->op == GGML_OP_SINKHORN_NORM);

    char base[256];
    char name[256];

    snprintf(base, 256, "kernel_sinkhorn_norm_%s", ggml_type_name(op->src[0]->type));
    snprintf(name, 256, "%s", base);
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_dsv4_hc_weighted_sum(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    snprintf(base, 256, "kernel_dsv4_hc_weighted_sum_%s", ggml_type_name(op->src[0]->type));
    snprintf(name, 256, "%s", base);
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_dsv4_hc_expand(ggml_metal_library_t lib, const ggml_tensor * op) {
    assert(op->op == GGML_OP_DSV4_HC_EXPAND);

    char base[256];
    char name[256];

    snprintf(base, 256, "kernel_dsv4_hc_expand_%s", ggml_type_name(op->src[0]->type));
    snprintf(name, 256, "%s", base);
