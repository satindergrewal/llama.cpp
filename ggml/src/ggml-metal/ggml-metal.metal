...
// ============================================================================================
// PAGED VARIANT OF THE CHAMPION FLASH-ATTENTION KERNEL
//
// Ported from ggml's Metal kernel_flash_attn_ext_impl (Georgi Gerganov and the ggml
// contributors), llama.cpp. Literal-copy port; the ONLY deltas are the K/V base addresses,
// which come from a paged block table instead of a linear offset.
//
// WHY this port exists, measured not assumed: the champion's threadgroup footprint is
// 10,240 B at C=64/D=128 with f16 KV and is FLAT IN nsg (the nsg term is multiplied by
// is_q). Every layout we wrote couples footprint to nsg -- LPK 21,632 via the padded K tile,
// MMA 28,928 via the O accumulator, which capped MMA at nsg=4 (measured, -14.8%). We were
// fighting an occupancy ceiling this kernel does not have.
//
// CONTRACT: C must equal the paged block_size, because the champion walks rows contiguously
// across its whole C-key chunk and paged rows are contiguous only WITHIN a block. Champion
// C = NCPSG = 64, so the pool must run bs=64. That is free: pool footprint is bs-invariant
// (measured 3.46 GiB at bs=16 and bs=32 alike).
// ============================================================================================
template<
    typename q_t,     // query types in shared memory
    typename q4_t,
...
void kernel_paged_champ_impl(
        ...
        device const char * pad,
        device const char * blk,
        device const int32_t * ptab,   // paged: block table  [seq*max_blocks + blk_idx]
        device const int32_t * plen,   // paged: ctx_lens per sequence
        device       char * dst,
        threadgroup  half * shmem_f16,
        uint3   tgpig,
        ushort  tiisg,
...
#define NS20 (FC_flash_attn_ext_ns20)
  ...
  //constexpr short PV16 = PV/16;
...

    {
        ...
        // loop over the KV cache
        // each simdgroup handles blocks of Q rows and C columns
        // PAGED: one iteration per paged block. plen[0] is this sequence's context length.
        const int n_kv_p = plen[0];
        for (int ic0 = 0; ; ++ic0) {
            int ic = ic0*C;
            if (ic >= n_kv_p) {
                break;
            ...
            if (is_same<kd4x4_t, k4x4_t>::value) {
                // we can read directly from global memory
                // ★ PAGED: ic0 is the BLOCK index (C == bs, host-enforced), so the base comes from the
                // block table instead of a linear offset. Rows inside a block are stride_token
                // apart -- already what NS10 carries, so nothing downstream changes.
                device      const k_t * pk = (device const k_t *) (k + (uint64_t) ptab[ic0]*args.nb13);
                threadgroup const q_t * pq = sq;
                threadgroup       s_t * ps = ss;
            ...
            // O = O + (Q*K^T)*V
            {
                ...
                if (is_same<vd4x4_t, v4x4_t>::value) {
                    ...
                    {
                        device const v_t * pv = (device const v_t *) (v + (uint64_t) ptab[ic0]*args.nb23);
...
// Ported from ggml's kernel_flash_attn_ext_vec (Georgi Gerganov and the ggml contributors).
// Args struct is field-identical to the MMA one, so the ENTIRE prefill port contract carries
// over unchanged. Unlike the MMA kernel this one has NO `blk` parameter -- so the skip-array
// trap that cost three debugging rounds on the prefill port cannot occur here.
// Every anchor above was verified present BEFORE transforming: attempt #1 reused the prefill
// script blind, mis-anchored, and broke the whole Metal library.
// ============================================================================================
template<
    typename q4_t,  // query types in shared memory
    typename k4_t,  // key types in shared memory
...
kernel void kernel_paged_champ_vec(
        ...
        device const char * sinks,
        device const char * pad,
        device const int32_t * ptab,   // paged: block table
        device const int32_t * plen,   // paged: ctx_lens per sequence
        device       char * dst,
        threadgroup  half * shmem_f16 [[threadgroup(0)]],
        uint3   tgpig[[threadgroup_position_in_grid]],
...
#define NS20 (FC_flash_attn_ext_vec_ns20)
  ...
  //threadgroup q_t   * sq  = (threadgroup q_t   *) (shmem_f16 +                      0*PK); // holds the query data
...

    {
        ...
        // PAGED: nb13/nb23 carry the BLOCK stride (host pins ne03=ne_12_3=1 so ikv3=0); V sits
        // at head (n_heads_kv + kv_h), which ne_12_2 carries. Identical to the prefill port.
        k += ikv2*args.nb12;
        v += (ikv2 + args.ne_12_2)*args.nb22;
    }
