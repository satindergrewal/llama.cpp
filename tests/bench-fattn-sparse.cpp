//
// Wall-clock benchmark: sparse top-k MMA flash attention (upstream PR #25917, the
// `use_top_k`-templated stock kernel) vs the mask-shaped formulation it is meant to
// replace, at real GLM-5.2 MLA shapes.
//
// Ported from llama.cpp-dsaport/tests/bench-fattn-dsa.cpp. The `gather` path of that
// bench (the standalone GGML_OP_FLASH_ATTN_EXT_DSA cuBLAS-gather op) does not exist in
// this tree and is REMOVED; its numbers are joined from the dsaport logs at report time.
//
// The paths, as llm_graph_context::build_attn() would build them:
//
//   mma_topk  ggml_flash_attn_ext(q, k, v, kq_mask_BASE, scale, 0, 0) @ GGML_PREC_F32
//             + ggml_flash_attn_ext_add_top_k(cur, top_k)
//             one node; the stock MMA kernel templated with use_top_k=true iterates
//             over top_k->ne[0] gathered rows: tile loads read KV + top_k[i]*stride_KV,
//             mask loads read mask[j*stride + top_k[i]]. The mask passed is the BASE
//             causal mask (zeros up to the causal limit, -INF beyond), NOT the
//             -INF-filled top-k mask: the kernel only ever reads it at the top-k
//             positions, where the mask path's constructed mask holds the same values.
//             Dispatch gate (ggml_cuda_flash_attn_ext_mma_f16_shall_use_top_k +
//             _may_use_top_k): DKQ=576/DV=512 ncols1=1 ncols2=16 (or 512/512 ncols2=8),
//             and (Q->ne[1]==1 && n_kv>=4096) || n_kv>=8192. If the gate fails the SAME
//             graph silently computes DENSE attention over the base causal mask, which
//             is numerically very different from top-k attention -- --verify detects
//             that and reports it as a fallback instead of a win/loss.
//
...
//
//   dense     stock FA over the BASE causal mask with NO top_k. Not a contender: it is
//             the fallback fingerprint (mma_topk output==dense output => gate fell
//             through) and the full-attention reference cost.
//
...
//
// Measurement notes (identical to the dsaport bench so numbers join cleanly):
//   * inputs (q_cur, k, kq_mask, top_k) are identical for all paths and live in
...
//     two separate processes produce bit-identical inputs, and inputs are bit-identical
//     with the dsaport bench at equal shapes (same streams, same seed): absmeans are
//     directly comparable across the two benches' logs.
//   * --verify runs mask, mma_topk and dense back to back in-process on the same inputs
//     and compares outputs elementwise (max/mean abs diff), classifying mma_topk as
//     SPARSE-OK / FELL-BACK-TO-DENSE / MISMATCH.
//
...
// best case for nothing; a real indexer's picks cluster.
//
// Deliberately NOT wired into tests/CMakeLists.txt (CUDA-only comparison, run by hand):
//
//   cd /mnt/nvme0/llama.cpp-sparsemma
//   g++ -O2 -std=c++17 -Iggml/include tests/bench-fattn-sparse.cpp \
//       -Lbuild/bin -lggml -lggml-base -Wl,-rpath,$PWD/build/bin -o build/bin/bench-fattn-sparse
//   CUDA_VISIBLE_DEVICES=0 ./build/bin/bench-fattn-sparse --list
//   CUDA_VISIBLE_DEVICES=0 ./build/bin/bench-fattn-sparse --case 0 --verify
//   CUDA_VISIBLE_DEVICES=0 ./build/bin/bench-fattn-sparse --case 0 --path mma_topk
//
...
enum path_kind {
    ...
    PATH_MASK,
    PATH_CONSTRUCT,
    PATH_DENSE,     // stock FA over the base causal mask, no top_k: fallback fingerprint
...
static const char * path_name(path_kind p) {
    switch (p) {
        case PATH_MMA_TOPK:  return "mma_topk";
        case PATH_MASK:      return "mask";
        case PATH_CONSTRUCT: return "construct";
        case PATH_DENSE:     return "dense";
...
struct run_result {
    ...
    int64_t n_nodes            = 0;
    std::vector<float> out_host;     // filled only when capture is requested (--verify)
};

// host-side mirror of the CUDA dispatch gate for the sparse top-k MMA kernel:
//   ggml_cuda_flash_attn_ext_mma_f16_may_use_top_k():  (DKQ==576 && DV==512 && ncols1==1 && ncols2==16)
//                                                   || (DKQ==512 && DV==512 && ncols1==1 && ncols2==8)
//   ggml_cuda_flash_attn_ext_mma_f16_shall_use_top_k(): ((Q->ne[1]==1 && K->ne[1]>=4096) || K->ne[1]>=8192)
// plus the fattn.cu routing that must reach those template instantiations at all
// (gqa_ratio % 16 == 0 for the 576/512 case). Used only to PREDICT the dispatch so a
// dense fallback is named instead of silently timed; ground truth is the --verify
// output comparison.
static const char * sparse_gate_prediction(const bench_case & c) {
    const bool shape_ok_576 = c.DK == 576 && c.DV == 512 && c.n_head % 16 == 0; // ncols2=16 instantiation
    const bool shape_ok_512 = c.DK == 512 && c.DV == 512 && c.n_head %  8 == 0; // ncols2=8  instantiation
    if (!shape_ok_576 && !shape_ok_512) {
        return "dense-predicted (no use_top_k instantiation for this DKQ/DV/gqa)";
    }
    if (!((c.n_tokens == 1 && c.n_kv >= 4096) || c.n_kv >= 8192)) {
        return "dense-predicted (shall_use_top_k n_kv gate)";
    }
    return "sparse-predicted";
}
...
static run_result run_one(ggml_backend_t backend, const bench_case & c, path_kind path,
    ...
    // ---- inputs (identical for every path, and bit-identical with the dsaport bench
    //      at equal shapes: same index-keyed PRNG streams, same seed) ---------------
    ...
    } else if (path == PATH_MMA_TOPK || path == PATH_DENSE) {
        // stock FA over the BASE causal mask. For mma_topk, ggml_flash_attn_ext_add_top_k()
        // attaches the top-k index tensor as src[5]; the CUDA dispatch then selects the
        // use_top_k=true kernel template (if the gate passes), which only reads K/V rows
        // and mask entries at the top_k positions. The base mask holds the same values at
        // those positions as the mask path's constructed mask (0 inside the causal range),
        // so both paths softmax over exactly the same index set.
        // For dense, the same node WITHOUT top_k: full attention over the causal range.
        out = ggml_flash_attn_ext(ctx_g, q, k, v, kq_mask, scale, 0.0f, 0.0f);
        ggml_flash_attn_ext_set_prec(out, GGML_PREC_F32);
        if (path == PATH_MMA_TOPK) {
            ggml_flash_attn_ext_add_top_k(out, top_k);
            ggml_set_name(out, "kqv_mma_topk");
        } else {
            ggml_set_name(out, "kqv_dense");
        }
    ...
    // ---- output checksum -------------------------------------------------------
    // identical inputs across processes => mma_topk and mask must agree here (and with
    // the dsaport gather/mask logs at equal shapes)
...
static void print_result(const bench_case & c, path_kind path, const run_result & R) {
    ...
    if (path == PATH_MMA_TOPK) {
        printf(" dispatch=\"%s\"", sparse_gate_prediction(c));
    }
    printf("\n");
}

// elementwise output comparison for --verify
struct diffstat {
    double  mx   = 0.0;
    double  mean = 0.0;
    int64_t n    = 0;
    int64_t nonfinite_mismatch = 0;
};

static diffstat diff_outputs(const std::vector<float> & a, const std::vector<float> & b) {
    diffstat d;
    d.n = (int64_t) std::min(a.size(), b.size());
    double s = 0.0;
    for (int64_t i = 0; i < d.n; ++i) {
        const bool fa = std::isfinite(a[i]);
        const bool fb = std::isfinite(b[i]);
        if (!fa || !fb) {
            if (fa != fb) {
                ++d.nonfinite_mismatch;
            }
            continue;
        }
        const double ad = std::fabs((double) a[i] - (double) b[i]);
        s += ad;
        if (ad > d.mx) {
            d.mx = ad;
        }
...
int main(int argc, char ** argv) {
    ...
    for (int i = 0; i < g_n_cases; ++i) {
        ...
        bench_case c0 = g_cases[i];
        if (ov_nkv)   c0.n_kv     = ov_nkv;
        if (ov_ntok)  c0.n_tokens = ov_ntok;
        if (ov_topk)  c0.top_k    = ov_topk;
        if (ov_nhead) c0.n_head   = ov_nhead;

        if (verify) {
            // correctness: mask, mma_topk and dense on bit-identical inputs, compared
            // elementwise. Timing from these runs is NOT the headline (in-process,
            // whatever --target says); use per-process --case/--path runs for timing.
            const run_result Rmask  = run_one(backend, c0, PATH_MASK,     seed, min_iters, target_s, verbose, true);
            const run_result Rmma   = run_one(backend, c0, PATH_MMA_TOPK, seed, min_iters, target_s, verbose, true);
            const run_result Rdense = run_one(backend, c0, PATH_DENSE,    seed, min_iters, target_s, verbose, true);
            print_result(c0, PATH_MASK,     Rmask);
            print_result(c0, PATH_MMA_TOPK, Rmma);
            print_result(c0, PATH_DENSE,    Rdense);

            if (!Rmask.supported || !Rmma.supported || !Rdense.supported) {
                printf("VERIFY tag=%s n_kv=%lld n_tokens=%lld top_k=%lld n_head=%lld verdict=REFUSED\n",
                       c0.tag, (long long) c0.n_kv, (long long) c0.n_tokens, (long long) c0.top_k,
                       (long long) c0.n_head);
                rc = 1;
                fflush(stdout);
                continue;
            ...
            const diffstat d_mm = diff_outputs(Rmma.out_host,  Rmask.out_host);  // the correctness claim
            const diffstat d_md = diff_outputs(Rmma.out_host,  Rdense.out_host); // fallback fingerprint
            const diffstat d_dm = diff_outputs(Rdense.out_host, Rmask.out_host); // discrimination power

            // fp16 tile arithmetic in a different summation order: elementwise agreement is
            // expected at ~1e-3; dense-vs-top-k disagreement is O(output magnitude). 1e-2
            // sits an order of magnitude from both.
            const double tol = 1e-2;
            const char * verdict = "MISMATCH";
            if (Rmma.out_nonfinite == 0 && d_mm.nonfinite_mismatch == 0 && d_mm.mx < tol) {
                verdict = "SPARSE-OK";
            } else if (d_md.mx < tol) {
                verdict = "FELL-BACK-TO-DENSE";
            }

            printf("VERIFY tag=%s n_kv=%lld n_tokens=%lld top_k=%lld n_head=%lld "
                   "mask_absmean=%.9g mma_absmean=%.9g dense_absmean=%.9g "
                   "maxdiff_mma_vs_mask=%.6g meandiff_mma_vs_mask=%.6g "
                   "maxdiff_mma_vs_dense=%.6g maxdiff_dense_vs_mask=%.6g "
                   "nonfinite_mask=%lld nonfinite_mma=%lld nonfinite_mismatch=%lld "
                   "dispatch=\"%s\" verdict=%s\n",
                   c0.tag, (long long) c0.n_kv, (long long) c0.n_tokens, (long long) c0.top_k,
                   (long long) c0.n_head,
                   Rmask.out_absmean, Rmma.out_absmean, Rdense.out_absmean,
                   d_mm.mx, d_mm.mean, d_md.mx, d_dm.mx,
                   (long long) Rmask.out_nonfinite, (long long) Rmma.out_nonfinite,
                   (long long) d_mm.nonfinite_mismatch,
                   sparse_gate_prediction(c0), verdict);
            fflush(stdout);
            if (strcmp(verdict, "SPARSE-OK") != 0) {
                rc = 1;
            }
            continue;
        }
